...
class VITFeatureExtractor(nn.Module):
    def __init__(self, model_name="vit_base_patch16_224.orig_in21k_ft_in1k", pretrained_cfg=None, use_input_norm=True, use_range_norm=False):
        super(VITFeatureExtractor, self).__init__()
        '''
        use_input_norm: If True, x: [0, 1] --> (x - mean) / std
        use_range_norm: If True, x: [0, 1] --> x: [-1, 1]
        '''
        self.model = create_model(model_name, pretrained=True, pretrained_cfg=pretrained_cfg)
    ...
    def __init__(self, feature_layer=[2,7,16,25,34], weights=[0.1,0.1,1.0,1.0,1.0], 
        ...
        if percept == "vgg":
            ...
            if percept == "vit":
                ...
                pretrained_cfg = {"file": "weights/jx_vit_base_p16_224-80ecf9dd.pth"}
            elif percept == "mae":
                model_name = "vit_base_patch16_224"
                pretrained_cfg = {
                    "file": "weights/mae.pth",
                    'num_classes': 5
                    }
            elif percept == "usfm":
                model_name = "beit_base_patch16_224"
                pretrained_cfg = {
                    "file": "weights/usfm.pth",
                    'num_classes': 5
                    }
            else:
                # pretrained_cfg = {"file": "/data3/jackeyguo/weights/USFM_20231222.pth"}
                assert 1 == 2, "error percept name: {}".format(percept)
            
            print(model_name, pretrained_cfg)
            self.vgg = VITFeatureExtractor(model_name=model_name, pretrained_cfg=pretrained_cfg,
                                           use_input_norm=use_input_norm, use_range_norm=use_range_norm)
