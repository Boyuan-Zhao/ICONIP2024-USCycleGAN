...
class BasicBlock(nn.Module):
    def __init__(self,inplanes, planes, stride = 1, attn_layer=None):
...
class ResidualBlock(nn.Module):
    def __init__(self, in_features, attn_layer=None):
        ...
        self.se = create_attn(attn_layer, in_features)
...
class FeatureExtractorWithInfo(nn.Module):
    def __init__(self, n_residual_blocks, in_features, attn_layer):
        ...
        for _ in range(n_residual_blocks):
            model_body_block1 += [ResidualBlock(in_features, attn_layer=attn_layer)]
            model_body_block2 += [ResidualBlock(in_features, attn_layer=attn_layer)]
            model_body_block3 += [ResidualBlock(in_features, attn_layer=attn_layer)]
            model_body_block4 += [ResidualBlock(in_features, attn_layer=attn_layer)]
...
class Generatorv2(nn.Module):
    def __init__(self, input_nc, output_nc, n_residual_blocks=8, attn_layer=None):
        ...
        # Main blocks
        self.feature_extractor = FeatureExtractorWithInfo(n_residual_blocks//4, in_features, attn_layer)

        # auxiliary block
        # num_classes=50
        model_body2 = []
        for _ in range(3):
            model_body2 += [BasicBlock(in_features, in_features, attn_layer=attn_layer)]
            model_body2 += [BasicBlock(in_features, in_features, attn_layer=attn_layer, stride=2)]
